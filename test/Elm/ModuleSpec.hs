...
moduleCode' elmVersion = unlines
    ...
    , "            , (\"Qux2\", Json.Decode.lazy (\\_ -> Json.Decode.map Qux2 (   (Json.Decode.field \"a\" (Json.Decode.int)) >>= \\pa ->    (Json.Decode.field \"test\" (localDecoder_a)) >>= \\ptest ->    Json.Decode.succeed {a = pa, test = ptest})))"
    , "            ]"
